...
namespace TownsPeople.GamePlay
{
    ...
    public class NPCGossipMemory : MonoBehaviour
    {
        ...
        // v26: OPTIONAL — implemented only by the Locomotion add-on's NPCFlockingBehavior.
        // While IsActive is true, PresentRumorInternal() skips presentation entirely (see
        // there). Resolved via plain GetComponent<T>() — no reflection needed, since the
        ...
        private string PresentRumorInternal(RumorTemplate rumor, bool showInWorldBubble)
        {
            ...
            // this NPC — skip presentation entirely, WITHOUT marking HasBeenPresented, so this
            // rumor remains eligible to actually present once that behavior ends rather than
            // being silently skipped forever. Learning (LearnRumor) is unaffected — this method
            ...
            {
                Debug.Log($"<color=grey>[NPCGossipMemory]</color> '{gameObject.name}' is in a priority behavior — presentation of '{rumor.RumorID}' suppressed (still eligible later).");
                return null;
            }
